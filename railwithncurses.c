/*****************************************************************************
 * Copyright (C) Ansh Bordia ansh979@gmail.com
 *
...
#define size 5      /*normal seating(3) + those alloted for waitlis(2)t*/
#define SizeCust 50

int count = 0, fraud = 0, lgn = 0, c2 = 0, noteac = -1, notesl = -1, looksl = 0, lookac = 0, csac = 0, cssl = 0;
char str_login[10];

typedef struct queue {          /* to store booking list in repective trains */
	int train_no;
	char uname[5][10];	
	int total;
}queue;

typedef struct store {		/*to write new customers*/
	int id;
	char uname[10];	
	char pass[10];
	int phone_no;
...
typedef struct storer {  	/*to store the previously read customer file*/
	int id;
	char uname[10];	
	char pass[10];
	int phone_no;
...
typedef struct tdata {       	/*stores train info*/
	int no;
	char name[50];
	char from[50];
	char to[50];
...
typedef struct tseats {		/*stores seat info*/
	int no;
	int sleeperseats;
	int acseats;
}tseats;
...
void checkUserName(char unm[]) {		/*prevents ID duplication*/
	int i = 0;
	char um[10];
	while (i < SizeCust) {
...
int CheckAvailibility(int id) {		/*check seats availiable to book in a train*/
	int i, n, ct = 0;
...
int findtrain() {
	move(0,0);
	clrtobot();
	if(lgn == 0) {    				/*if user has not logged in*/
	...
	switch(choice) {		/*repeats if no trains are found*/
		case 1 : findtrain();
			 break;
		case 2 : exit(1);
...
int updateseats() {		/*rewrites to the file the new no. of seats*/
	int i;	
...
int readqueuesl() {			/*reads the list of people who have booked their tickets in sleeper*/
	int n;
...
int readqueueac() {			/*reads the list of people who have booked their tickets in AC*/
	int n;
...
int writequeuesl(int n) {		/*updates sleeper queue of already booked trains*/
	int i = 0;
...
int writequeueac(int n) {			/*updates AC queue of already booked trains*/
	int i = 0;
...
int appendqueueac() {			/*appends if booking is made in a new train(AC)*/
	int i = 0;
...
int appendqueuesl() {			/*appends if booking is made in a new train(SL)*/
	int i = 0;
...
int Enqueuesl(char login[], int no) {		/*function that actually puts in the queue*/
	notesl = -1;	
	int n;
	n = readqueuesl();
	int i;
...
void Dequeuesl(int no) {    /*removes from queue*/
	int n;
	n = readqueuesl();
	int i;
	int look2 = 0;
...
int frd(int f) {		/*blocks more that 2 wrong attempts*/
	if(f > 2) { 
